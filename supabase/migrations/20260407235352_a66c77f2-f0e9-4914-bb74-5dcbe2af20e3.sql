...
INSERT INTO public.site_settings (key, value) VALUES
  ('store_name', 'Minha Loja de Makes'),
  ('whatsapp_number', '5500000000000'),
  ('whatsapp_message', 'Olá! Gostaria de finalizar meu pedido:'),
  ('logo_url', ''),
...
CREATE POLICY "Authenticated users can update store images" ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'store-images');
CREATE POLICY "Authenticated users can delete store images" ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'store-images');
